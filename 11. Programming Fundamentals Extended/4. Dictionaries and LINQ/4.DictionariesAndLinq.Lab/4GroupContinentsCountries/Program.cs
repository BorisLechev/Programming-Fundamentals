namespace _4GroupContinentsCountries
{
    using System;
    ...
    public class GroupContinentsCountries
    {
        ...
        {
            var database = new SortedDictionary<string, SortedDictionary<string, SortedSet<string>>>();
            ...
            for (int i = 0; i < number; i++)
            {
                ...
                {
                    database.Add(continent, new SortedDictionary<string, SortedSet<string>>());
                }

                if (!database[continent].ContainsKey(country))
                {
                    database[continent].Add(country, new SortedSet<string>());
                }
